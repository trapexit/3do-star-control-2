...
typedef MAP_INFO	near *NPMAP_INFO;
typedef MAP_INFO	far *LPMAP_INFO;
#endif /* MAPPING */
...
typedef struct
{
    ...
    PROC_PARAMETER(
    void (**func_array), (pClipRect, PrimPtr),
	    ARG		(PRECT		pClipRect)
	    ARG_END	(LPPRIMITIVE	PrimPtr)
    );
    FONT		Font;
...
typedef CONTEXT_DESC	near *NPCONTEXT_DESC;
typedef CONTEXT_DESC	far *LPCONTEXT_DESC;
