...
typedef INTERNAL_PRIM_DESC	near *NPINTERNAL_PRIM_DESC;
typedef INTERNAL_PRIM_DESC	far *LPINTERNAL_PRIM_DESC;

typedef LPINTERNAL_PRIM_DESC	INTERNAL_PRIM_DESCPTR;
...
typedef INTERNAL_PRIMITIVE	near *NPINTERNAL_PRIMITIVE;
typedef INTERNAL_PRIMITIVE	far *LPINTERNAL_PRIMITIVE;

STAMP	_save_stamp;

PROC(STATIC
BOOLEAN GetFrameValidRect, (pValidRect, pOldHot),
    ARG		(PRECT		pValidRect)
    ARG_END	(HOT_SPOT	*pOldHot)
)
{
    COORD	hx, hy;
    HOT_SPOT	OldHot;
...
PROC(STATIC
void ClearBackGround, (pClipRect),
...
void DrawBatch, (lpBasePrim, PrimLinks, BatchFlags),
    ...
    ARG_END	(register BATCH_FLAGS	BatchFlags)
)
{
    RECT	ValidRect;
    HOT_SPOT	OldHot;
			...
			: GetSuccLink (GetPrimLinks (lpPrim)))
	{
		...
		Prim = *(LPINTERNAL_PRIMITIVE)lpWorkPrim;
		lpWorkPrim = (LPPRIMITIVE)&Prim;
		...
		case COMPOSITEFILL_PRIM:
		    ...
		    if ((Prim.Object.Composite.FrameList =
			    (LPFRAME)Prim.Object.Composite.frame) == 0
...
ProcessStamp:
		...
		case POLY_PRIM:
		case POLYFILL_PRIM:
		    ClipRect = ((LPPOLYGON_DESC)lpWorkPrim->Object.Polygon)->BoundRect;
		    break;
	    }
