...
static void
stop_phys_channel (ulong Channel)
{
    SuspendTasking ();
    ...
    SendSignal (ServiceItem, ServiceSignal);
    ResumeTasking ();
}
...
int ChannelPlaying (ulong Channel)
{
	...
	if (ChanDesc[Channel].FinishTime)
	{
	    if (GetTimeCounter () <= ChanDesc[Channel].FinishTime)
		return (1);
	    stop_phys_channel (Channel);
	}
...
{
    OpenAudioFolio ();
    ServiceSignal = AllocSignal (0);
    for (;;)
    {
	int	Channel;

	service_waiting = 1;
	WaitSignal (ServiceSignal);
...
// only for 1.3.1?????
	...
	if ((ChanDesc[i].SampleIns = AllocInstrument (SampleTmp, 100)) > 0)
	{
	    ...
	    sprintf (buf, "Input%d", i + REDBOOK_CHANNELS);
...
int UninitSound (void)
{
    ...
    while (!service_waiting)
	Yield ();

    DeleteThread (ServiceItem);
    ...
    _uninit_CD (); // before I free mixer, since StopTrack may use it
