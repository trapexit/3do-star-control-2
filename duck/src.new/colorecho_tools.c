/*
	File:		colorecho_tools.c

	Contains:	"Color Echo" Tools
				Video feedback using LRForm cels
	...
	Change History (most recent first):
		 ...
		 <2>	 7/28/93	JAY		add IO parameter block to CopyVRAMPages() and WaitVBL for Dragon
									Tail release.
...
/*
** Copy CCB to DMA type memory in case program is loaded into non-DMAable memory.
** Currently all memory is accessable by DMA but future machines may extend
** beyond the 26 bit DMA address limit and could break this code.
*/
	ce->ce_CCB = AllocMem( sizeof(CCB), MEMTYPE_DMA );
	if( ce->ce_CCB == NULL )
	{
...
/* Do Sport transfer of displayed image to target image. */
	if (ce->ce_IfSport)
	{
		...
		CopyVRAMPages( VRAMIOReq, sc->sc_Bitmaps[sc->sc_curScreen]->bm_Buffer,   /* dest */
