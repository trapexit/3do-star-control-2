...
// #define TRACE(s)   kprintf("ST: %s\n", s)
#define TRACE(s) ;

#define NUMSCREENS 2
#define kTileBackGroundCel  "$tuner/Background Cel"
#define kListCel            "$tuner/List Cel"
#define kHighlightCel       "$tuner/Highlight Cel"
#define kOptionsCel         "$tuner/Options Cel"
#define kAlertCel           "$tuner/Alert Cel"
#define kHelpCel            "$tuner/Help Cel"
...
#ifdef TOYS_FOR_BOB
#define IS_HORDE_FILE(f)	(strlen (f) == 11 \
	&& (f[0] == 's' || f[0] == 'S') \
	&& (f[1] == 'g' || f[1] == 'G') \
	&& f[8] == '.' && f[2] < '4')

#define IS_STARCON_FILE(f)	(0)
	
static bool
GetT4BComment (char *originalName)
{
    if (IS_HORDE_FILE (originalName))
    {
	int		Scenario;
	unsigned long	Val;
	char		*buf, *b;
	char		*LocationNames[] = { "Shimto", "Alburga", "Buuzal", "Kar-Nyar", "Vesh" };
		
	buf = originalName;
	b = &buf[3];
	while (*b == '0' && b != &buf[8])
	    b++;
	Val = (unsigned long)atoi (b);
	Scenario = buf[10] - '0';
	if (buf[9] == 'X')
	{
	    sprintf (gComment.comment, "The Horde\n%s, Castle, $%lu",
		    LocationNames[Scenario], Val);
	}
	else
	{
	    int	Year;
	    ...
	    Year = buf[9] - '0' + 1;
	    sprintf (gComment.comment, "The Horde\n%s, Year %d, $%lu",
		    LocationNames[Scenario], Year, Val);
	}
    }
    else if (IS_STARCON_FILE (originalName))
    {
    }
    else
	return (FALSE);
...
FileRecord *fr;
    ...
    // …before disposing of the current
    LHDisposeCell(cellP);
}
...
static bool
get_input (ControlPadEventData *cpe, bool wait, uint32 mask)
{
    int32	i;
    
    for (i = 1; i <= 2; i++)
    {
	if (GetControlPad (i, FALSE, cpe) > 0 && (cpe->cped_ButtonBits & mask))
	    return (TRUE);
    }
...
#else
    while (!get_input (&cpe, TRUE, ControlA | ControlC))
	;
#endif
...
#ifdef TOYS_FOR_BOB
    else if (!strncmp (originalName, "starcon2", strlen ("starcon2")))
    {
	*comment = "Star Control II Saved Game";
	return (originalName);
    }
    else if (!strncmp (&originalName[strlen (originalName) - strlen (".mle")],
	    ".mle", strlen (".mle")))
    {
	*comment = "Super Melee Team";
	return (originalName);
    }
    else if (!strncmp (originalName, "melee.cfg", strlen ("melee.cfg")))
    {
	*comment = "Super Melee Setup";
	return (originalName);
    }
#endif
...
Err err;
    ...
    if (err >= 0)
    {
        ...
        if (err >= 0)
        {
            ...
            err = InitEventUtility(2, 0, LC_Observer);
            if (err >= 0)
            {
                ...
                gFileFont = LoadFont((char *)"$tuner/Sans Serif 14", MEMTYPE_ANY);
                if (!gFileFont)
                {
...
#ifdef TOYS_FOR_BOB
	sc.sc_Screens[0] = screens[0];
	sc.sc_Screens[1] = screens[1];
	sc.sc_nScreens = 2;
	
        FadeFromBlack (&sc, NUM_FADE_STEPS);
#endif
        if (err >= 0)
        {
            while (TRUE)
...
#else
                result = get_input (&controlPadEvent, FALSE, ~0);
#endif
...
#else
		    // wait until a or b is hit
		    while (!get_input (&cpe, FALSE, ControlC))
			;
#endif
...
#ifdef TOYS_FOR_BOB
            FadeToBlack (&sc, NUM_FADE_STEPS * 3);
#else
            FadeToBlack(screens);
#endif
