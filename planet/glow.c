...
void
blt_cel (void *data, long len, long pixc)
{
    CCB	*ccb;
    ...
    *cur_ccb = *ccb;
	...
	if (xor)
	    SET_CEL_FLAGS (cur_ccb, CCB_PXOR);
	cur_ccb->ccb_PIXC = pixc;
    }
    cur_ccb->ccb_NextPtr = (void *)MakeCCBRelative (&cur_ccb->ccb_NextPtr, cur_ccb + 1);
    
    cur_ccb->ccb_XPos = cel_pt[0].pt_X << 16;
    cur_ccb->ccb_YPos = cel_pt[0].pt_Y << 16;
...
#define HEIGHT	100
    ...
    chrbuf = LoadFile ("planetchr.cel", &chrsize, MEMTYPE_ANY);
    lumbuf = LoadFile ("planetlum.2", &lumsize, MEMTYPE_ANY);
    
    SET_CEL_PTS (STARTX, STARTY);
    blt_cel (chrbuf, chrsize, 0);
	
    SET_CEL_PTS (STARTX + WIDTH, STARTY);
    blt_cel (lumbuf, lumsize, 0);
    
    SET_CEL_PTS (STARTX, STARTY + HEIGHT);
    blt_cel (chrbuf, chrsize, 0);
    
    if (argc > 1)
    {
	pixc = atoi (*(argv + 1));
	printf ("pixc is 0x%04x  ", pixc);
	if (argc > 2)
	{
	    xor = 1;
	    printf ("xor is on");
    ...
    {
	// primary source is frame buffer (full strength)
	// seconday source is (inverted?) mask (full strength)
	// subtract
	pixc = 0x85c2;
    }
    
    blt_cel (lumbuf, lumsize, PIXC_DUP (pixc));
