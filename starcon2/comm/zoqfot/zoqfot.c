...
static LOCDATA	zoqfot_desc =
{
    ...
    ZOQFOTPIK_PLAYER_PHRASES,		/* PlayerPhrases */
    3,					/* NumAnimations */
    {
	{	/* Eye blink */
	    1,				/* StartIndex */
...
PROC(STATIC
void near SelectAlien, (index, which_alien),
    ARG		(COUNT		index)
    ARG_END	(BYTE		which_alien)
)
{
    switch (which_alien)
    {
...
PROC_LOCAL(
void far ZoqFotHome, (R),
    ARG_END	(RESPONSE_REF	R)
);

PROC(STATIC
void far ZoqFotInfo, (R),
    ARG_END	(RESPONSE_REF	R)
)
{
    PUSH_CONTEXT
...
strcpy(aiff_folder, "comm/zoqfot/zoqfo");
