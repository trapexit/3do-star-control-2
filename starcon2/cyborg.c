...
)
{
		...
		|| (time_x_1 < 0 && dx <= time_x_1))
	{
		...
		else
		{
			...
			Control1.EndPoint.x = WORLD_TO_DISPLAY (Pt1.x);
			Control1.EndPoint.y = WORLD_TO_DISPLAY (Pt1.y);
			if (when = DrawablesIntersect (&Control0,
...
PROC(STATIC
void near InitCyborg, (StarShipPtr),
...
PROC(STATIC
void near ship_movement, (ShipPtr, EvalDescPtr),
...
|| (ObjectsOfConcern->ObjectPtr->state_flags & CREW_OBJECT)
	    ...
	    {
		if (ShipFired = ship_weapons (ShipPtr,
...
)
{
    ...
    if (ship_delta_facing = NORMALIZE_FACING (ANGLE_TO_FACING (angle) - f))
    {
