...
PROC(STATIC
void DrawCursor, (curs_x, curs_y),
    ARG		(COORD	curs_x)
    ARG_END	(COORD	curs_y)
)
{
    STAMP	s;
...
PROC(STATIC
void near DrawAutoPilot, (pDstPt),
    ARG_END	(PPOINT	pDstPt)
)
{
    SIZE	dx, dy,
		xincr, yincr,
...
PROC(STATIC
void GetSphereRect, (StarShipPtr, pRect, pRepairRect),
    ARG		(EXTENDED_SHIP_FRAGMENTPTR	StarShipPtr)
    ARG		(PRECT				pRect)
    ARG_END	(PRECT				pRepairRect)
)
{
    long	diameter;
	...
	t.lpStr = GetStringAddress (locString);
...
PROC(STATIC
void DrawStarMap, (race_update, pClipRect),
    ARG		(COUNT	race_update)
...
#define GRID_DELTA	500
    ...
    SetContextDrawState (DEST_PIXMAP | DRAW_SUBTRACTIVE);
    BatchGraphics ();
...
PROC(STATIC
void EraseCursor, (curs_x, curs_y),
    ARG		(COORD	curs_x)
    ARG_END	(COORD	curs_y)
)
{
...
PROC(STATIC
BOOLEAN DoMoveCursor, (InputState, pMS),
...
#define MAX_ACCEL_DELAY	(ONE_SECOND / 8)
	...
	pMS->Initialized = MAKE_WORD (
		ASCIIToScan (SK_INSERT),
		ASCIIToScan (SK_DELETE)
		);
...
#ifdef NEVER
	ZoomStarMap (KeyDown (LOBYTE (pMS->Initialized))
		- KeyDown (HIBYTE (pMS->Initialized)));
#else
...
UpdateCursorInfo:
		...
		{
		    last_buf = buf[0];
		    DrawSISMessage ((LPSTR)buf);
		}
		sprintf (buf, "Fuel: %u.%u",
...
PROC(STATIC
void near RepairMap, (update_race, pNextRect, pLastRect),
    ARG		(COUNT	update_race)
    ARG		(PRECT	pLastRect)
    ARG_END	(PRECT	pNextRect)
)
{
    RECT	r;
...
DoneSphereMove:
	    ...
	    if (delta = StarShipPtr->ShipInfo.actual_strength
		    - StarShipPtr->ShipInfo.known_strength)
	    {
		if (!MapDrawn)
		{
...
PROC(
BOOLEAN DoFlagshipCommands, (InputState, pMS),
    ARG		(INPUT_STATE	InputState)
    ARG_END	(PMENU_STATE	pMS)
)
{
