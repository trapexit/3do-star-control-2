...
PROC(
void GenerateRotatedScans, (FramePtr, pscan_block, angle),
    ARG		(FRAMEPTR	FramePtr)
    ARG		(PSCAN_BLOCK	pscan_block)
    ARG_END	(COUNT		angle)
)
{
    ...
    lpBase = (LPBYTE)FramePtr + FramePtr->DataOffs;
    if (0)
	lpBase += width * height;
...
#ifdef DEBUG
{
PROC_GLOBAL(DWORD AnyButtonPress, (CheckSpecial), ARG_END (BOOLEAN CheckSpecial));
