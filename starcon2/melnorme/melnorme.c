...
PROC_LOCAL(
COUNT far initialize_pump_up, (ShipPtr, PumpUpArray),
    ARG		(LPELEMENT	ShipPtr)
    ARG_END	(HELEMENT	far PumpUpArray[])
);
...
static void	far
pump_up_postprocess (ElementPtr)
LPELEMENT	ElementPtr;
{
    PUSH_CONTEXT
    {
...
static void	far
pump_up_collision (ElementPtr0, pPt0, ElementPtr1, pPt1)
...
static void	far
confusion_collision (ElementPtr0, pPt0, ElementPtr1, pPt1)
LPELEMENT	ElementPtr0, ElementPtr1;
PPOINT		pPt0, pPt1;
{
    PUSH_CONTEXT
...
PROC(STATIC
COUNT far initialize_confusion, (ShipPtr, ConfusionArray),
    ARG		(LPELEMENT	ShipPtr)
    ARG_END	(HELEMENT	far ConfusionArray[])
)
{
    PUSH_CONTEXT
    {
...
PROC(STATIC
COUNT far initialize_test_pump_up, (ShipPtr, PumpUpArray),
    ARG		(LPELEMENT	ShipPtr)
    ARG_END	(HELEMENT	far PumpUpArray[])
)
{
...
COUNT		ConcernCounter;
{
		...
		&& StarShipPtr->RaceDescPtr->ship_info.energy_level >= SPECIAL_ENERGY_COST)
	{
	    ...
	    StarShipPtr->ship_input_state = old_input_state
		    | (StarShipPtr->ship_input_state & SPECIAL);
	}
...
static void	far
melnorme_postprocess (ElementPtr)
LPELEMENT	ElementPtr;
{
    PUSH_CONTEXT
    {
